...
var (
	hosts         = []string{"117.51.148.112:2181"} // the zookeeper hosts
	basePath      = "/locker"                       //the application znode path
	lockerTimeout = 5 * time.Second                 // the maximum time for a locker waiting
	zkTimeOut     = 20 * time.Second                // the zk connection timeout
)
